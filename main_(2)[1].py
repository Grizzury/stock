...
async def on_message(message):
    ...
    if 'The pokémon is ' in content:
        ...
        #If still not found, check for Eevee Pokémon
        if not solution:
             solution = solve(content, 'eevee.txt')
             if solution:
                await move_channel (channel, solution, 'Eevee')
     
        #If still not found, check for Zorua Pokémon 
        if not solution:
             solution = solve(content, 'zorua.txt')
             if solution:
                await move_channel (channel, solution, 'Zorua')
